...
# === Problem 1
class RectangularRoom(Position, object):
    """
    ...
    def isPositionInRoom(self, pos):
        ...
        if pos.getX() >= self.width or pos.getX() < 0:
            return False
        elif pos.getY() >= self.height or pos.getY() < 0:
            return False
        return True


class Robot(RectangularRoom, object):
    """
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        NewPosition = self.pos.getNewPosition(self.getRobotDirection(), self.speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        for j in range(num_robots):
            robots = robots.append(robot_type(room, speed))
        while float(room.getNumCleanedTiles()/room.getNumTiles()) < min_coverage:
            # anim.update(room, robots)
            robots.updatePositionAndClean()
            num_steps += num_robots
        time_steps.append(num_steps)
        # anim.done()
    print(time_steps)
    return float(sum(time_steps)/num_trials)

avg = runSimulation(1, 1.0, 5, 5, 1, 30, StandardRobot)
print avg
