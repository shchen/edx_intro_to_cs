...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for k in range(300):
            patient.update()
            SumTimeTrial[k] += patient.getTotalPop()
    for i in range(len(SumTimeTrial)):
        SumTimeTrial[i] /= float(numTrials)
    pylab.figure()
    pylab.plot(range(300), SumTimeTrial, linewidth=2)
            ...
            if random.random() < self.maxBirthProb * (1 - popDensity):
                for key in self.resistances.keys():
                    if random.random() < self.mutProb:
                        newResistances[key] = not newResistances[key]
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in updateViruses:
            try:
                offsprings.append(virus.reproduce(popDensity, self.drugList))
            except NoChildException:
                pass
        self.viruses += offsprings
        return len(self.viruses)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for k in range(150):
            patient.update()
            SumTimeTrial[k] += patient.getTotalPop()
            SumTimeTrialwithDrug[k] += patient.getResistPop(['guttagonol'])

        #resistances['guttagonol'] = True
        patient.addPrescription('guttagonol')
        for l in range(150,300):
            patient.update()
            SumTimeTrial[l] += patient.getTotalPop()
            SumTimeTrialwithDrug[l] += patient.getResistPop(['guttagonol'])

    for i in range(len(SumTimeTrial)):
        SumTimeTrial[i] /= float(numTrials)
        SumTimeTrialwithDrug[i] /= float(numTrials)

    pylab.figure()
    pylab.plot(range(300), SumTimeTrial, 'r-', label='SimpleVirus')
    pylab.plot(range(300), SumTimeTrialwithDrug, 'b-', label='ResistantVirus')
